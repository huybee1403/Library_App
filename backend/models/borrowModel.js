...
const Borrow = {
    borrowBook: async ({ userId, bookId, borrowDays = 14 }) => {
        return db.transaction(async (connection) => {
            const [bookRows] = await connection.query("SELECT id, title FROM books WHERE id = ?", [bookId]);
            if (!bookRows.length) {
    ...
    extendBorrow: async ({ borrowId, userId, extraDays = 7, isAdmin = false }) => {
        return db.transaction(async (connection) => {
            ...
            const [overdueRows] = await connection.query("SELECT due_date < CURDATE() AS is_overdue FROM borrows WHERE id = ?", [borrowId]);
            ...
            if (reservationByOthersRows[0].total > 0) {
                throw createError("Khong the gia han vi sach da co nguoi dat truoc", 409);
            }

            await connection.query("UPDATE borrows SET due_date = DATE_ADD(due_date, INTERVAL ? DAY) WHERE id = ?", [extraDays, borrowId]);

            const [updatedRows] = await connection.query("SELECT id, due_date FROM borrows WHERE id = ?", [borrowId]);
    ...
    getBookStatus: async (bookId) => {
        const bookRows = await db.query("SELECT id, title FROM books WHERE id = ?", [bookId]);
        if (!bookRows.length) {
            throw createError("Khong tim thay sach", 404);
        }
        ...
        const reservationCountRows = await db.query("SELECT COUNT(*) AS total FROM reservations WHERE book_id = ?", [bookId]);
